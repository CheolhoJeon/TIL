...
위의 상호 작용 모델(interaction model) 외에도 응용 프로그램 및 시스템 효율성에 도움이 될 수 있는 다른 동작(behavior)이 있음

### 3-1. Single-Response vs. Multi-Response

* 단일 응답(Single-Reponse)과 다중 응답(Multi-Response)의 주요 차이점 중 하나는 RSocket 스택이 애플리케이션에 데이터를 전달하는 방식임:
* 단일 응답은 여러 프레임에 걸쳐 전달될 수 있으며, 다중화된 여러 메시지를 스트리밍하는 더 큰 RSocket 연결의 일부임
* 그러나 단일 응답은 응답의 전체 데이터가 수신되었을 때, 애플리케이션이 데이터를 취하는 것을 의미함
* 반면에 다중 응답은 데이터를 조금씩 나눠서 전달함
* 이를 통해 사용자는 다중 응답을 염두에 두고 서비스를 설계할 수 있으며, 클라이언트는 첫 번째 청크(chunk)를 수신하는 즉시 데이터 처리를 시작할 수 있음

### 3-2. Bi-Directional

* RSocket은 클라이언트와 서버가 모두 요청자 또는 응답자 역할을 할 수 있는 양방향(bi-directional) 요청을 지원함
* 이를 통해 클라이언트(예: 사용자 장치)가 서버의 요청에 대한 응답자 역할을 할 수 있음



* 예를 들어, 서버는 추적 디버그 정보, 상태 등에 대해 클라이언트를 쿼리할 수 있음
* 이것은 수백만/10억 명의 클라이언트가 가끔씩만 필요할 수도 있는 데이터를 지속적으로 제출하도록 하는 대신 필요할 때 서버 측에서 쿼리를 허용함으로써 인프라 확장 요구 사항을 줄이는 데 사용할 수 있음
